...
static void jlgr_thread_event(jl_t* jl, void* data) {
	...
	switch(packet->id) {
		case JLGR_COMM_RESIZE:
			...
			break;
		case JLGR_COMM_KILL:
			jl_io_print(jl, "Thread exiting....");
			jl_gr->draw.rtn = 1;
			break;
		case JLGR_COMM_INIT:
			JL_IO_DEBUG(jl,"Running prg's graphical init func....");
			packet->fn(jl);
			jl_gr->draw.rtn = 2;
			break;
		case JLGR_COMM_SEND:
...
static void jlgr_thread_draw_init__(jl_t* jl) {
	...
	JL_IO_DEBUG(jl, "Creating the window....");
	jl_dl_init__(jl_gr);
	JL_IO_DEBUG(jl, "Loading default graphics from package....");
	jl_sg_inita__(jl_gr);
	JL_IO_DEBUG(jl, "Setting up OpenGL....");
	jl_gl_init__(jl_gr);
	JL_IO_DEBUG(jl, "Load graphics....");
	jl_gr_init__(jl_gr);
	JL_IO_DEBUG(jl, "Creating Mouse / Taskbar sprites....");
	jlgr_gui_init_(jl_gr);
	JL_IO_DEBUG(jl, "Setting Variables....");
	jl_sg_initb__(jl_gr);
	JL_IO_DEBUG(jl, "Looking for init packet....");
	// Wait until recieve init packet....
	while(jlgr_thread_draw_event__(jl) != 2);
	JL_IO_DEBUG(jl, "Sending finish packet....");
...
void jlgr_thread_init(jl_gr_t* jl_gr) {
	jl_gr->running = 1;
	jl_gr->thread = jl_thread_new(jl_gr->jlc, "JL_Lib/Graphics",
		jlgr_thread_draw);
}

void jlgr_thread_kill(jl_gr_t* jl_gr) {
	jl_gr->running = 0;
	jl_thread_old(jl_gr->jlc, jl_gr->thread);
}
